...
已完成：页面骨架与 tokens（P4-01）；业务 RPC 脊柱与壳通道（P4-02）；转换树与三态选择（P4-03）；设置表单/输出矩阵/预览（P4-04）；自定义选择器/按钮动作链、hook 开关、DialogHost（P4-05）；RunControls 开始/取消/重置接线、阶段与结果文案（P4-06，UI06/EX03 前端侧）；日志面板分页/筛选/复制导出/ANSI 安全富文本与 getLogs 游标契约（P4-07，UI07/EX04 前端侧）；树虚拟化（RAC Virtualizer）、帧预算 64MiB（100k 快照过通道）、三引擎浏览器测试与主题/axe 基线（P4-08，UI03/UI08 浏览器层）；依赖/产物扫描（无旧 UI 运行依赖）、真实 WebView2 桌面 E2E（9/9）、G4 报告（P4-09，Windows 实测范围通过）。
...
已完成：发行目标清单/targets 与 runtime-manifest schema、命名和矩阵生成器（P5-01）；单份 Node 获取校验与 production 组装器、发行布局自定位接线（P5-02，PK07 本机部分）；Windows 双配置/NSIS、WebView2 原生预检与发布布局自定位、双变体实构（P5-03，bootstrap 29MiB/offline 232MiB、负载集合一致、旧版口径 -70%）；Linux 预检引导器与在线 DEB（P5-05，WSL Debian 13 构建/安装/WSLg 冒烟）；离线自含 AppImage 原型（P5-06，141MiB 内嵌 WebKitGTK 闭包，musl 剔除修复）；macOS 配置/脚本与 D5 最低系统 13.5 定稿（P5-04，实构阻塞于 macOS 主机）；SPDX SBOM 与哈希/许可清单（P5-07，签名阻塞于受控环境）；大小分解/产物扫描/CI 工作流 CI-01..07（P5-09，Actions 全 SHA 固定，推送后验证）。

阻塞（VM/实机/受控环境）：P5-04 macOS 实构；P5-07 签名公证；P5-08 升级/修复/卸载 VM 矩阵；P5-10 G5 全目标真机验收（本机已有 win x64 + WSL Debian 13 双变体证据）。
