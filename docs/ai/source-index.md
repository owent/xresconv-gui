...
| 主题 | 来源 | 结论 | status |
| --- | --- | --- | --- |
| 包管理器 | `package.json`、`.yarnrc.yml`、`yarn.lock`、工作树状态 | 当前工作树已迁移 Yarn 4 并删除另两份 JS 锁文件；唯一 JS 锁为 yarn.lock，Cargo.lock 继续服务 Tauri | current（工作树事实，完整 P1 验收待执行） |
| 构建/打包 | `gulpfile.js`、`package.json` scripts | gulp 5 任务驱动；`@electron/packager` 打包到 `out/`；macOS 必须 `asar=false` | current |
| 测试与 lint | `package.json`、`biome.json`、`apps/desktop/vitest.config.ts`、`tests/`、`packages/contracts` | Biome/Vitest/契约测试已按 D6 收敛：schema→TS/Ajv 纯 Node 链路（无 Cargo），单元/契约 32 例与桌面 E2E（WebView2）在 win32/x64 通过（P1-04/P1-06/P1-08 记录） | current（P1 已验收；Linux/macOS 待 CI/实机） |
...
- P1-00 收敛审计已完成（[记录](../plan/records/P1-00.md)）；Node 业务/契约测试独立于 Cargo，Tauri 壳仅保留窗口/CLI/对话框/握手桥。完整桌面、安装、脚本隔离与性能验收仍待 P2–P6 执行，不能以骨架测试替代。
- Yarn 4/锁文件迁移已在工作树出现；后续核验 `--immutable`、动态 require 与 CI，不重复删除或回退用户已有改动。
...
复核日期：2026-09-23。主计划保存先前核验的依赖/Actions 版本快照；本轮针对 D6 重新核验 Tauri 必需 Rust 边界、Node sidecar/child_process 和 XML 候选。**P0 旧版基线已有记录，新架构、安装器、兼容性与性能尚未完整验收。** 详细合同见 [执行计划索引](../plan/README.md)，不在本索引复制任务正文。
...
| Node XML 候选 | [fast-xml-parser 官方仓库](https://github.com/NaturalIntelligence/fast-xml-parser)、[npm 元数据](https://registry.npmjs.org/fast-xml-parser/latest) | 本轮候选 5.11.1；校验、属性/实体/CDATA/顺序选项须按 CF01 和 BD-07 实测，解析成功不等于合法 XML | 每次依赖升级 | P3-01 | 来源/版本已核验，兼容实现待执行 |
