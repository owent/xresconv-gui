...
- **显示设置独立面板**：状态条右侧 fieldset 面板（与详情按钮分列不同面板）；新增四区字体字号设置（全局/界面控件/树/日志；family 经 queryLocalFonts 枚举+datalist 输入过滤，size 白名单 6–48px；CSS 变量 `--font-<area>-*` 应用；Rust 白名单校验+控制字符清洗）。
...
- **Java 运行时检查（F06/F12 恢复旧版 conv_env_check）**：backend `checkJava` RPC（java -version 探测+版本≥8+64-Bit 判定+下载指引，契约枚举扩展）；解析链 `XRESCONV_JAVA → JAVA_HOME/bin/java → PATH`，**与 java-runner 转换执行共用同一解析**（显示=执行）；状态条展示（来源标注+版本首行），不满足时醒目警告+推荐发行版。
...
- **显示设置**：主题三态（跟随系统/亮/暗，`<html data-theme>`，tokens.css 三态）+ 上次转换列表自动加载；持久化到可执行程序目录 `display-settings.json`（壳层 read/write_display_settings 命令，白名单校验）；入口=状态条 ⚙ 按钮；
