...
截至 P4-04a 的一致性约束：加载成功后才更换配置路径；加载互斥，旧会话的排队操作/响应不能覆盖新配置或故障状态；树版本跨 reload 单调推进；活动运行必须完成日志收尾后才接纳下一次 run。设置输入/输出为独立快照，矩阵重估释放旧限制，preview 按 `(outputDir, rename)` 二元组分组。长期 guardian 健康视图使用监督状态，不套用 one-shot NodeHealth。回归见 [增量审查](records/REVIEW-P2-P5-2026-09-24.md)。
